...
def carregar_dados():
    try:
        ...
        if dados.empty:
            return pd.DataFrame(columns=CAMPOS_COLUNAS)
        # Garante ordem das colunas mesmo se editar no sheets
        dados = dados.reindex(columns=CAMPOS_COLUNAS)
        return dados
    except Exception as e:
...
if pagina == "Cadastro/Editar Viagem":
    ...
    # Dicionário de fallback para novo registro ou campos vazios
    for campo in CAMPOS_COLUNAS:
        if campo not in registro:
            if campo in ["diesel_abastecido_belem", "diesel_chegada_barco", "manobras", "mca",
                         "saldo_diesel_atual", "abastecimento", "qtd_transbordo", "saldo_diesel_viagem"]:
                registro[campo] = 0.0
            else:
                registro[campo] = ""
    ...
    with st.form("form_cadastro"):
        ...
        with col5:
            ...
            # Tempo total de porto (calculo automático)
            tempo_porto = ""
            try:
                dt_chegada = datetime.combine(data_chegada_manaus, datetime.strptime(hora_chegada_manaus, "%H:%M").time())

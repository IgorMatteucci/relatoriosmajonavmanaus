...
if pagina == "Cadastro/Editar Viagem":
    ...
    def get_data_or_none(valor):
        ...
        with col2:
            ...
            # --- DATA SAÍDA BELÉM ---
            _data_saida_belem = get_registro("data_saida_belem", "")
            if not _data_saida_belem or str(_data_saida_belem).lower() in ("", "nan", "none", "nat"):
                data_saida_belem = st.date_input("Data Saída Belém", value=None, key="data_saida_belem")
            else:
                data_saida_belem = st.date_input("Data Saída Belém", value=pd.to_datetime(_data_saida_belem).date(), key="data_saida_belem")
        ...
        with col3:
            # --- DATA CHEGADA MANAUS ---
            _data_chegada_manaus = get_registro("data_chegada_manaus", "")
            if not _data_chegada_manaus or str(_data_chegada_manaus).lower() in ("", "nan", "none", "nat"):
                data_chegada_manaus = st.date_input("Data Chegada Manaus", value=None, key="data_chegada_manaus")
            else:
                data_chegada_manaus = st.date_input("Data Chegada Manaus", value=pd.to_datetime(_data_chegada_manaus).date(), key="data_chegada_manaus")
        ...
        with col5:
            ...
            # --- DATA SAÍDA MANAUS ---
            _data_saida_manaus = get_registro("data_saida_manaus", "")
            if not _data_saida_manaus or str(_data_saida_manaus).lower() in ("", "nan", "none", "nat"):
                data_saida_manaus = st.date_input("Data Saída Manaus", value=None, key="data_saida_manaus")
            else:
                data_saida_manaus = st.date_input("Data Saída Manaus", value=pd.to_datetime(_data_saida_manaus).date(), key="data_saida_manaus")

...
if pagina == "Cadastro/Editar Viagem":
    ...
    # Define todos os campos com valor default caso não exista ainda
    campos = [
        "id_viagem", "nome_barco", "balsa1", "balsa2",
        "data_saida_belem", "hora_saida_belem", "diesel_abastecido_belem",
        "data_chegada_manaus", "hora_chegada_manaus", "horas_navegadas_balsa", "diesel_chegada_barco",
    ...
    for campo in campos:
        if campo not in registro:
            registro[campo] = "" if campo not in ["diesel_abastecido_belem", "diesel_chegada_barco",
                                                  "saldo_diesel_atual", "abastecimento", "qtd_transbordo",
                                                  "saldo_diesel_viagem"] else 0.0
    ...
    with st.form("form_cadastro"):
        ...
        with col6:
            ...
            qtd_transbordo = st.number_input("Quantidade (Litros) Transbordados", min_value=0.0, format="%.2f", value=float(registro["qtd_transbordo"]) if registro["qtd_transbordo"] != "" else 0.0)
            barco_transbordo = st.text_input("Barco envolvido no transbordo (de ou para)", value=registro["barco_transbordo"])

        # Lógica do saldo final considerando transbordo

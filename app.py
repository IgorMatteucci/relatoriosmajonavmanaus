...
# --- Lista de colunas sincronizada (INCLUI BALSA SAÍDA MANAUS e BALSA2 SAÍDA MANAUS)
COLUNAS = [
    ...
    "balsa_saida_manaus", "balsa2_saida_manaus",  # NOVAS COLUNAS
...
def carregar_dados():
    try:
        ...
        # Garante todas as colunas, preenche vazios se faltar
        for col in COLUNAS:
...
def append_google_sheets(novo_registro):
    try:
        spreadsheet = gc.open_by_key(SHEET_URL)
        worksheet = spreadsheet.worksheet(SHEET_NAME)
        worksheet.append_row([novo_registro.get(col, "") for col in COLUNAS])
        st.success("Registro adicionado ao Google Sheets com sucesso!")
    except Exception as e:
        st.warning(f"Não foi possível adicionar ao Google Sheets: {e}")

def update_google_sheets(id_viagem, novo_registro):
    try:
        spreadsheet = gc.open_by_key(SHEET_URL)
        worksheet = spreadsheet.worksheet(SHEET_NAME)
        all_records = worksheet.get_all_records()
        row_index = next((i for i, row in enumerate(all_records) if str(row.get("id_viagem")) == str(id_viagem)), None)
        if row_index is not None:
            worksheet.update(f"A{row_index+2}:{chr(65+len(COLUNAS)-1)}{row_index+2}",
                             [[novo_registro.get(col, "") for col in COLUNAS]])
            st.success("Registro atualizado no Google Sheets!")
        else:
            st.warning("ID não encontrado para atualização no Google Sheets!")
    except Exception as e:
        st.warning(f"Não foi possível atualizar no Google Sheets: {e}")

def delete_google_sheets(id_viagem):
    try:
        spreadsheet = gc.open_by_key(SHEET_URL)
        worksheet = spreadsheet.worksheet(SHEET_NAME)
        all_records = worksheet.get_all_records()
        ...
        if row_index is not None:
            ...
            st.success("Registro excluído do Google Sheets!")
        else:
            st.warning("ID não encontrado para exclusão no Google Sheets!")
    except Exception as e:
        st.warning(f"Não foi possível excluir no Google Sheets: {e}")
...
if pagina == "Cadastro/Editar Viagem":
    ...
    if not dados_encontrados.empty:
        ...
        if indices:
            ...
            registro.update({"diesel_abastecido_belem": 0.0, "diesel_chegada_barco": 0.0,
                             "saldo_diesel_atual": 0.0, "abastecimento": 0.0, "saldo_diesel_viagem": 0.0})
    else:
        registro = {col: "" for col in COLUNAS}
        registro.update({"diesel_abastecido_belem": 0.0, "diesel_chegada_barco": 0.0,
                         "saldo_diesel_atual": 0.0, "abastecimento": 0.0, "saldo_diesel_viagem": 0.0})
    ...
    with st.form("form_cadastro"):
        ...
        balsa2_saida_manaus = st.text_input("Balsa2 Saída Manaus", value=registro.get("balsa2_saida_manaus", ""))  # NOVO CAMPO
    ...
    if submitted:
        novo_registro = {
            ...
            "balsa_saida_manaus": balsa_saida_manaus,  # NOVO CAMPO
            "balsa2_saida_manaus": balsa2_saida_manaus,  # NOVO CAMPO
        ...
        if indice_editar is not None:
            ...
            salvar_dados(dados)
            update_google_sheets(id_viagem, novo_registro)
            ...
            salvar_dados(dados)
            append_google_sheets(novo_registro)
            st.success("✅ Novo registro salvo com sucesso!")
        st.rerun()

    if indice_editar is not None and excluir:
        id_viagem_excluir = dados.loc[indice_editar, "id_viagem"]
        dados = dados.drop(indice_editar).reset_index(drop=True)
        salvar_dados(dados)
        delete_google_sheets(id_viagem_excluir)
        st.success(f"🚨 Viagem ID {id_viagem_excluir} excluída com sucesso!")
        st.rerun()

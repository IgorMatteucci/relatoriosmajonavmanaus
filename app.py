...
def salvar_google_sheets(dados):
    try:
        # Ordena colunas antes de salvar para garantir layout correto
        colunas_ordenadas = [
            ...
            "data_saida_manaus", "balsa_saida_manaus", "balsa2_saida_manaus", "hora_saida_manaus", "tempo_total_porto",
            "manobras", "mca",
            "saldo_diesel_atual", "abastecimento", "transbordo", "quantidade_transbordo", "barco_transbordo",
            "saldo_diesel_viagem"
        ]
...
def carregar_dados():
    try:
        ...
        # Ajusta colunas caso esteja em branco
        if dados.empty:
        ...
        # Garante ordem das colunas ao carregar
        dados = dados.reindex(columns=[
            "id_viagem", "nome_barco", "balsa1", "balsa2",
            "data_saida_belem", "hora_saida_belem", "diesel_abastecido_belem",
            "data_chegada_manaus", "hora_chegada_manaus", "horas_navegadas_balsa", "diesel_chegada_barco",
            "data_saida_manaus", "balsa_saida_manaus", "balsa2_saida_manaus", "hora_saida_manaus", "tempo_total_porto",
...
if pagina == "Cadastro/Editar Viagem":
    ...
    if not dados_encontrados.empty:
        ...
        if indices:
            ...
            registro = {
                ...
                "data_saida_belem": "",
                "hora_saida_belem": "",
                "diesel_abastecido_belem": 0,
                "data_chegada_manaus": "",
                "hora_chegada_manaus": "",
                "horas_navegadas_balsa": "",
                "diesel_chegada_barco": 0,
                "data_saida_manaus": "",
                "balsa_saida_manaus": "",
                "balsa2_saida_manaus": "",
                "hora_saida_manaus": "",
                ...
                "manobras": 0,
                "mca": 0,
                "saldo_diesel_atual": 0,
                "abastecimento": 0,
                "transbordo": "",
                "quantidade_transbordo": 0,
                "barco_transbordo": "",
                "saldo_diesel_viagem": 0
            }
    else:
        registro = {
            "id_viagem": "",
    ...
    with st.form("form_cadastro"):
        ...
        with col2:
            ...
            hora_saida_belem = st.text_input("Hora Saída Belém (HH:MM)", value=str(registro["hora_saida_belem"]))  # Manual!
        ...
        with col3:
            ...
            hora_chegada_manaus = st.text_input("Hora Chegada Manaus (HH:MM)", value=str(registro["hora_chegada_manaus"]))  # Manual!
        with col4:
        ...
        with col5:
            ...
            balsa_saida_manaus = st.text_input("Balsa Saída Manaus", value=registro.get("balsa_saida_manaus", ""))
            balsa2_saida_manaus = st.text_input("Balsa2 Saída Manaus", value=registro.get("balsa2_saida_manaus", ""))
            hora_saida_manaus = st.text_input("Hora Saída Manaus (HH:MM)", value=str(registro["hora_saida_manaus"]))
        with col6:
            tempo_total_porto = st.text_input("Tempo Total de Porto (h)", value=str(registro["tempo_total_porto"]))
            manobras = st.number_input("Manobras (L)", min_value=0.0, format="%.0f", value=float(registro.get("manobras", 0)))
            mca = st.number_input("MCA (L)", min_value=0.0, format="%.0f", value=float(registro.get("mca", 0)))
        ...
        transbordo = st.selectbox("Transbordo", ["", "Recebeu", "Passou"], index=0 if registro.get("transbordo", "") == "" else (1 if registro.get("transbordo", "") == "Recebeu" else 2))
        quantidade_transbordo = st.number_input("Quantidade Transbordo (L)", min_value=0.0, format="%.0f", value=float(registro.get("quantidade_transbordo", 0)))
        barco_transbordo = st.text_input("Barco do Transbordo", value=registro.get("barco_transbordo", ""))

        # Saldo de diesel para viagem (abastecimento + saldo atual + transbordo se recebeu)
        saldo_diesel_viagem = saldo_diesel_atual + abastecimento - manobras - mca
        if transbordo == "Recebeu":
            saldo_diesel_viagem += quantidade_transbordo
    ...
    if submitted:
        novo_registro = {
            ...
            "tempo_total_porto": tempo_porto_str if 'tempo_porto_str' in locals() else tempo_total_porto,
            "manobras": manobras,
            "mca": mca,
            "saldo_diesel_atual": saldo_diesel_atual,
            "abastecimento": abastecimento,
            "transbordo": transbordo,
            "quantidade_transbordo": quantidade_transbordo,

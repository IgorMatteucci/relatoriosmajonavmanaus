    ...
    try:
        ...
        if dados.empty:
            return pd.DataFrame(columns=[
                ...
                "data_saida_manaus", "balsa_saida_manaus", "balsa2_saida_manaus",
                "hora_saida_manaus", "tempo_total_porto", "saldo_diesel_atual", "abastecimento",
...
if pagina == "Cadastro/Editar Viagem":
    ...
    with st.form("form_cadastro"):
        ...
        with col2:
            ...
            hora_saida_belem = st.time_input("Hora Saída Belém", value=pd.to_datetime(registro["hora_saida_belem"], format="%H:%M").time() if registro["hora_saida_belem"] else datetime.now().time())
        ...
        with col3:
            ...
            hora_chegada_manaus = st.time_input("Hora Chegada Manaus", value=pd.to_datetime(registro["hora_chegada_manaus"], format="%H:%M").time() if registro["hora_chegada_manaus"] else datetime.now().time())
        with col4:
        ...
        with col5:
            ...
            balsa2_saida_manaus = st.text_input("Balsa2 Saída Manaus (opcional)", value=registro["balsa2_saida_manaus"])
            ...
            hora_saida_manaus = st.time_input("Hora Saída Manaus", value=pd.to_datetime(registro["hora_saida_manaus"], format="%H:%M").time() if registro["hora_saida_manaus"] else datetime.now().time())
        with col6:
            ...
            transbordo = st.selectbox("Transbordo", options=["", "Recebeu", "Passou"], index=0 if registro["transbordo"] == "" else (1 if registro["transbordo"] == "Recebeu" else 2))
            quantidade_transbordo = st.number_input("Quantidade Transbordo (L)", min_value=0.0, format="%.2f", value=float(registro["quantidade_transbordo"]))
            barco_transbordo = st.text_input("Barco (De/Para)", value=registro["barco_transbordo"])

        saldo_diesel_viagem = saldo_diesel_atual + abastecimento
        if transbordo == "Recebeu":
            saldo_diesel_viagem += quantidade_transbordo
        st.text_input("Saldo de Diesel para Viagem (L)", value=format_brl(saldo_diesel_viagem), disabled=True)
    ...
    if dados.empty:
        ...
        # Formatar as colunas de combustíveis para exibição
        for col in ["diesel_abastecido_belem", "diesel_chegada_barco", "saldo_diesel_atual", "abastecimento", "quantidade_transbordo", "saldo_diesel_viagem"]:
            if col in dados.columns:
                dados[col] = dados[col].apply(format_brl)

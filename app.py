    ...
    try:
        ...
        if dados.empty:
            return pd.DataFrame(columns=[
                ...
                "transbordo", "quantidade_transbordo", "barco_transbordo",
                "saldo_diesel_viagem"
            ])
        return dados
    except Exception as e:
...
if pagina == "Cadastro/Editar Viagem":
    ...
    registro_padrao = {
        ...
        "abastecimento": 0.0,
        "transbordo": "",
        "quantidade_transbordo": 0.0,
        "barco_transbordo": "",
        "saldo_diesel_viagem": 0.0
    }
    for k in registro_padrao:
        if k not in registro:
            registro[k] = registro_padrao[k]
    ...
    with st.form("form_cadastro"):
        ...
        with col2:
            ...
            data_saida_belem = st.date_input(
                "Data Saída Belém",
                value=pd.to_datetime(registro["data_saida_belem"]) if registro["data_saida_belem"] else datetime.today()
            )
            hora_saida_belem = st.text_input("Hora Saída Belém (hh:mm)", value=str(registro["hora_saida_belem"]))
            diesel_abastecido_belem = st.number_input(
                "Diesel Abastecido em Belém (L)",
                min_value=0.0,
                format="%.0f",
                value=float(registro["diesel_abastecido_belem"])
            )
        ...
        with col3:
            data_chegada_manaus = st.date_input(
                "Data Chegada Manaus",
                value=pd.to_datetime(registro["data_chegada_manaus"]) if registro["data_chegada_manaus"] else datetime.today()
            )
            hora_chegada_manaus = st.text_input("Hora Chegada Manaus (hh:mm)", value=str(registro["hora_chegada_manaus"]))
        with col4:
            diesel_chegada_barco = st.number_input(
                "Diesel Chegada Barco (L)",
                min_value=0.0,
                format="%.0f",
                value=float(registro["diesel_chegada_barco"])
            )

        # Cálculo Horas Navegadas Balsa
        try:
            ...
            horas_navegadas_str = str(int(diff.total_seconds() // 3600))
        except:
            horas_navegadas_str = ""
        ...
        with col5:
            data_saida_manaus = st.date_input(
                "Data Saída Manaus",
                value=pd.to_datetime(registro["data_saida_manaus"]) if registro["data_saida_manaus"] else datetime.today()
            )
            balsa_saida_manaus = st.text_input("Balsa Saída Manaus", value=registro["balsa_saida_manaus"])
            balsa2_saida_manaus = st.text_input("Balsa2 Saída Manaus", value=registro["balsa2_saida_manaus"])
            hora_saida_manaus = st.text_input("Hora Saída Manaus (hh:mm)", value=str(registro["hora_saida_manaus"]))
        with col6:
            saldo_diesel_atual = st.number_input(
                "Saldo Diesel Atual (L)",
                min_value=0.0,
                format="%.0f",
                value=float(registro["saldo_diesel_atual"])
            )
            abastecimento = st.number_input(
                "Abastecimento (L)",
                min_value=0.0,
                format="%.0f",
                value=float(registro["abastecimento"])
            )
            transbordo = st.selectbox("Transbordo", ["", "Recebeu", "Passou"], index=["", "Recebeu", "Passou"].index(registro["transbordo"]) if registro["transbordo"] in ["", "Recebeu", "Passou"] else 0)
            quantidade_transbordo = st.number_input(
                "Quantidade Transbordo (L)",
                min_value=0.0,
                format="%.0f",
                value=float(registro["quantidade_transbordo"])
            )
            barco_transbordo = st.text_input("Barco do Transbordo", value=registro["barco_transbordo"])

        # Saldo Diesel Viagem (lógica do transbordo)
        saldo_diesel_viagem = saldo_diesel_atual + abastecimento
        if transbordo == "Recebeu":
            saldo_diesel_viagem += quantidade_transbordo
        elif transbordo == "Passou":
            saldo_diesel_viagem -= quantidade_transbordo

        st.text_input("Saldo de Diesel para Viagem (L)", value=format_litros(saldo_diesel_viagem), disabled=True)

        try:
            dt_chegada = datetime.combine(data_chegada_manaus, datetime.strptime(hora_chegada_manaus, "%H:%M").time())
            dt_saida_man = datetime.combine(data_saida_manaus, datetime.strptime(hora_saida_manaus, "%H:%M").time())
            tempo_porto_str = str(int((dt_saida_man - dt_chegada).total_seconds() // 3600))
        except:
            tempo_porto_str = ""

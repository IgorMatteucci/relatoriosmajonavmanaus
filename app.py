    ...
    try:
        ...
        if dados.empty:
            return pd.DataFrame(columns=[
                ...
                "data_saida_manaus", "balsa_saida_manaus", "balsa2_saida_manaus", "hora_saida_manaus", 
                "tempo_total_porto", "saldo_diesel_atual", "abastecimento",
                "transbordo", "quantidade_transbordo", "de_ou_para", "saldo_diesel_viagem"
            ])
        return dados
    except Exception as e:
        st.warning(f"Erro ao carregar dados do Google Sheets: {e}")
        # fallback para o Excel local caso Sheets falhe
...
if pagina == "Cadastro/Editar Viagem":
    ...
    # Lista de todos os campos e os campos numéricos
    campos = [
        "id_viagem", "nome_barco", "balsa1", "balsa2", "data_saida_belem", "hora_saida_belem", "diesel_abastecido_belem",
        "data_chegada_manaus", "hora_chegada_manaus", "horas_navegadas_balsa", "diesel_chegada_barco",
        "data_saida_manaus", "balsa_saida_manaus", "balsa2_saida_manaus", "hora_saida_manaus",
        "tempo_total_porto", "saldo_diesel_atual", "abastecimento",
        "transbordo", "quantidade_transbordo", "de_ou_para", "saldo_diesel_viagem"
    ...
    campos_numericos = [
        "diesel_abastecido_belem", "diesel_chegada_barco", "saldo_diesel_atual",
        "abastecimento", "quantidade_transbordo", "saldo_diesel_viagem"
    ]
    for campo in campos:
        if campo not in registro:
            registro[campo] = 0.0 if campo in campos_numericos else ""
    ...
    with st.form("form_cadastro"):
        ...
        with col6:
            ...
            quantidade_transbordo = st.number_input("Quantidade Transbordo (L)", min_value=0.0, format="%.0f", value=float(registro["quantidade_transbordo"]))
            de_ou_para = st.text_input("Barco de/para transbordo", value=registro["de_ou_para"])
        ...
        st.text_input("Saldo de Diesel para Viagem (L)", value=f"{int(saldo_diesel_viagem):,}".replace(",", "."), disabled=True)
    ...
    if submitted:
        novo_registro = {
            ...
            "balsa1": balsa1,
            "balsa2": balsa2,
            "data_saida_belem": data_saida_belem.strftime("%d-%m-%Y"),
            "hora_saida_belem": hora_saida_belem.strftime("%H:%M"),
            "diesel_abastecido_belem": int(diesel_abastecido_belem),
            "data_chegada_manaus": data_chegada_manaus.strftime("%d-%m-%Y"),
            "hora_chegada_manaus": hora_chegada_manaus.strftime("%H:%M"),
            "horas_navegadas_balsa": horas_navegadas_str,
            "diesel_chegada_barco": int(diesel_chegada_barco),
            "data_saida_manaus": data_saida_manaus.strftime("%d-%m-%Y"),
            ...
            "saldo_diesel_atual": int(saldo_diesel_atual),
            "abastecimento": int(abastecimento),
            "transbordo": transbordo,
            "quantidade_transbordo": int(quantidade_transbordo),
            "de_ou_para": de_ou_para,
            "saldo_diesel_viagem": int(saldo_diesel_viagem)
        }
        if indice_editar is not None:
